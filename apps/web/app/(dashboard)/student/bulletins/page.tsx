...
export default function StudentBulletinsPage() {
  ...
  // Récupérer l'ID étudiant depuis /students/me
  useEffect(() => {
    Promise.all([
      ...
      console.error(err);
      setError('Impossible de charger votre profil étudiant.');
    });
  }, []);
  ...
  useEffect(() => {
    ...
    setAnnualData(null);
    Promise.all([
      academicApi.get(`/bulletins/${studentId}/data?academic_year_id=${academicYear.id}&trimester=${selectedTrimester}`),
      academicApi.get(`/bulletins/${studentId}/data-annual?academic_year_id=${academicYear.id}`),
    ]).then(([triRes, annualRes]) => {
      setBulletinData(triRes.data.data);
      setAnnualData(annualRes.data.data);
    }).catch(console.error)
      .finally(() => setLoading(false));
  }, [studentId, selectedTrimester, academicYear]);
  ...
  const handleDownloadTri = async () => {
    if (!academicYear || !studentId) return;
    setGenerating('tri');
    try {
      const name = `${bulletinData?.student?.last_name}_${bulletinData?.student?.first_name}`;
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          {bulletinData ? `${bulletinData.student.last_name} ${bulletinData.student.first_name} · ${bulletinData.student.class_name}` : academicYear?.name}
        </p>
      </div>
      ...
      <Card className="mb-6">
        <div className="flex gap-4 p-4 items-end">
          ...
          <p className="text-sm text-gray-400 pb-2">Année scolaire : <span className="font-medium text-gray-600">{academicYear?.name}</span></p>
        </div>
      </Card>
      ...
      {!studentId ? (
        ...
        <>
          {/* Bannière moyenne trimestre */}
          <div className={cn('rounded-xl p-5 mb-6 border flex items-center justify-between', getMentionBg(bulletinData.generalAverage))}>
            <div>
              <p className="font-semibold text-gray-900 text-lg">
                {['1er', '2ème', '3ème'][parseInt(selectedTrimester) - 1]} Trimestre
              </p>
              <p className="text-sm text-gray-500">{bulletinData.mention}</p>
            </div>
            <div className="text-right">
              <p className={cn('text-4xl font-black', getMentionColor(bulletinData.generalAverage))}>
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={!!generating}
              className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50"
            >
              {generating === 'tri' ? (
